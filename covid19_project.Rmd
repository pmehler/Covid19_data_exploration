...
# CREATE FUNCTION TO MAKE DATA SETS
...
us <- confirmed_cases %>% 
  ...
  gather(key = X1.22.20, value = cases, 5:60) %>% 
...
hubei %>% 
  ...
  geom_vline(xintercept = as.Date("1.23.20",format="%m.%d.%y"), 
                color = "red", size=1) +
  xlim(as.Date("1.22.20",format="%m.%d.%y"),as.Date("2.9.20",format="%m.%d.%y")) +
  ylim(0,32000) +
  geom_smooth()
